...
class SurvivorGame(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        textLayout.addWidget(self.textWindow, 1, 0)
        ...
        # self.stringInput.setMaxLength(1)
    ...
    def startGame(self):
        ...
        self.day = 1
        # 게임룰을 설명하는 초기 설정
        self.textWindow.append("""공습경보가 울렸고 당신은 방공호로 대피했습니다.
...
게임을 진행하려면 다음 버튼을 눌러 주세요""")
    ...
    def InputClicked(self):
        self.answer = self.stringInput.text()
        self.textWindow.append("")
        if (game_event_input[self.event_number] == "yes 혹은 no를 입력해주세요 :"):
            if self.answer == "yes":
            ...
            self.textWindow.append(game_event_no[self.event_number])
            self.food += game_event_no_food[self.event_number]
        ...
        if self.InputCounter == 2:
            ...
            self.day += 1
            self.food -= 1
            print("남은 식량 :" + str(self.food) + "개 ,오늘은 " + str(self.day) + "일차입니다.")
            self.InputCounter = 0
